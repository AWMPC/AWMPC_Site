...
assert.match(bible, /\.fab-panel \{[\s\S]*display: flex;[\s\S]*opacity: 0;[\s\S]*visibility: hidden;[\s\S]*pointer-events: none;[\s\S]*background: var\(--float-nav-bg\);[\s\S]*box-shadow: var\(--float-nav-shadow\);[\s\S]*backdrop-filter: var\(--hdr-blur\);[\s\S]*-webkit-backdrop-filter: var\(--hdr-blur\);[\s\S]*transition: opacity 180ms ease, transform 180ms ease, visibility 0s linear 180ms;/);
...
assert.match(bible, /<div class="fab-section-label">Recent reading<\/div>[\s\S]*<div id="history-menu" class="fab-history-menu"><\/div>\s*<div class="fab-section-label">Search<\/div>\s*<button class="dd-item" id="btn-search" type="button">[\s\S]*<span class="dd-label">Search<\/span>/);
