...
assert.match(bible, /\.fab-panel \{[\s\S]*display: flex;[\s\S]*opacity: 0;[\s\S]*visibility: hidden;[\s\S]*pointer-events: none;[\s\S]*background: var\(--float-nav-bg\);[\s\S]*box-shadow: var\(--float-nav-shadow\);[\s\S]*transition: opacity 180ms ease, transform 180ms ease, visibility 0s linear 180ms;/);
...
assert.match(bible, /<div class="font-slider-wrap">[\s\S]*<\/div>\s*<button class="dd-item" id="btn-search" type="button">[\s\S]*<span class="dd-label">Search<\/span>[\s\S]*<div class="fab-section-label">Recent reading<\/div>[\s\S]*<div id="history-menu" class="fab-history-menu"><\/div>/);
...
assert.match(bible, /\.fab-main \{[\s\S]*padding: 0;[\s\S]*overflow: hidden;/);
assert.match(bible, /\.fab-main \.profile-avatar \{[\s\S]*width: 100%;[\s\S]*height: 100%;[\s\S]*max-width: none;[\s\S]*border-radius: 0;[\s\S]*object-fit: cover;[\s\S]*border: 0;/);
