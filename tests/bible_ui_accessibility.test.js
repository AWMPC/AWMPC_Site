...
assert.match(bible, /class="theme-mode-button display-size-button" type="button" data-display-step="0" aria-pressed="false">Small<\/button>/);
assert.match(bible, /class="theme-mode-button display-size-button" type="button" data-display-step="1" aria-pressed="true">Medium<\/button>/);
assert.match(bible, /class="theme-mode-button display-size-button" type="button" data-display-step="2" aria-pressed="false">Large<\/button>/);
assert.doesNotMatch(bible, /Font Size/);

assert.match(bible, /\.fab-panel \{[\s\S]*display: flex;[\s\S]*position: fixed;[\s\S]*right: 12px;[\s\S]*bottom: calc\(76px \+ env\(safe-area-inset-bottom, 0px\)\);[\s\S]*gap: var\(--display-panel-gap\);[\s\S]*padding: var\(--display-panel-padding\);[\s\S]*background: var\(--float-nav-bg\);[\s\S]*border: var\(--card-border\);[\s\S]*border-radius: var\(--radius-md\);[\s\S]*box-shadow: var\(--float-nav-shadow\);[\s\S]*backdrop-filter: var\(--hdr-blur\);[\s\S]*-webkit-backdrop-filter: var\(--hdr-blur\);[\s\S]*isolation: isolate;[\s\S]*transition: opacity 180ms ease, transform 180ms ease, visibility 0s linear 180ms;/);
...
assert.match(bible, /<div class="fab-card display-size-wrap">[\s\S]*<div class="fab-card-title">Display Size<\/div>[\s\S]*data-display-step="0"[\s\S]*Small[\s\S]*data-display-step="1"[\s\S]*Medium[\s\S]*data-display-step="2"[\s\S]*Large/);
