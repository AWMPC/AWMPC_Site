...
assert.match(bible, /\.bottom-chrome \{[\s\S]*left: 12px;[\s\S]*right: 12px;[\s\S]*bottom: calc\(8px \+ env\(safe-area-inset-bottom, 0px\)\);[\s\S]*align-items: center;[\s\S]*padding: 6px;[\s\S]*min-height: 52px;[\s\S]*background: var\(--float-nav-bg\);[\s\S]*border: var\(--card-border\);[\s\S]*border-radius: var\(--radius-md\);[\s\S]*box-shadow: var\(--float-nav-shadow\);[\s\S]*backdrop-filter: var\(--hdr-blur\);/);
