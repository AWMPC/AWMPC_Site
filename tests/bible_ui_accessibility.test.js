...
assert.match(bible, /\.view-inner \{[\s\S]*padding-bottom: var\(--bottom-chrome-clearance\);/);
...
assert.match(bible, /function updateBottomChromeClearance\(\) \{[\s\S]*var rect = bottomChrome\.getBoundingClientRect\(\);[\s\S]*var clearance = Math\.max\(88, viewportHeight - rect\.top \+ 16\);[\s\S]*document\.documentElement\.style\.setProperty\('--bottom-chrome-clearance', Math\.ceil\(clearance\) \+ 'px'\);[\s\S]*\}/);
