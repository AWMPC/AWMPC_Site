...
assert.match(bible, /\.fab-panel \{[\s\S]*display: flex;[\s\S]*gap: 10px;[\s\S]*padding: 12px;[\s\S]*background: var\(--float-nav-bg\);[\s\S]*box-shadow: var\(--float-nav-shadow\);[\s\S]*backdrop-filter: saturate\(180%\) var\(--hdr-blur\);[\s\S]*-webkit-backdrop-filter: saturate\(180%\) var\(--hdr-blur\);[\s\S]*transition: opacity 180ms ease, transform 180ms ease, visibility 0s linear 180ms;/);
...
assert.match(bible, /\.fab-card\.history-card \{ flex: 1 1 auto; min-height: 130px;/);
assert.match(bible, /\.fab-history-menu \{ flex: 1 1 auto; min-height: 0; overflow-y: auto; display: grid; gap: 6px; \}/);
...
assert.match(bible, /<div class="fab-card history-card">[\s\S]*<div class="fab-card-title">Recent Reading<\/div>[\s\S]*<div id="history-menu" class="fab-history-menu"><\/div>[\s\S]*<\/div>\s*<div class="fab-card search-card">[\s\S]*<div class="fab-card-title">Search<\/div>[\s\S]*<button class="dd-item" id="btn-search" type="button">/);
