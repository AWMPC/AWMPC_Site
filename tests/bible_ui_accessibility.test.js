...
assert.match(bible, /<span class="app-version-label">Version <span id="app-version">3\.0\.0<\/span><\/span><button type="button" id="footer-signout" class="dd-signout-button" hidden>Sign Out<\/button>/);
...
assert.match(bible, /\.profile-card \{[\s\S]*--profile-photo-height: calc\(var\(--display-control-height\) \+ \(var\(--display-panel-padding\) \* 4\)\);[\s\S]*position: relative;[\s\S]*overflow: hidden;[\s\S]*min-height: 92px;[\s\S]*padding: 0 12px 0 0;/);
...
assert.match(bible, /\.app-version \{[\s\S]*display: grid;[\s\S]*grid-template-columns: 1fr auto 1fr;[\s\S]*align-items: center;[\s\S]*gap: var\(--display-panel-gap\);[\s\S]*padding: var\(--display-panel-gap\) 16px max\(2px, env\(safe-area-inset-bottom, 0px\)\);[\s\S]*font-size: max\(12px, calc\(var\(--display-control-font-size\) - 6px\)\);[\s\S]*white-space: nowrap;/);
assert.match(bible, /\.app-version-label \{[\s\S]*grid-column: 2;/);
assert.match(bible, /\.app-version \.dd-signout-button \{[\s\S]*grid-column: 3;[\s\S]*justify-self: end;[\s\S]*font-size: max\(13px, calc\(var\(--display-control-font-size\) - 5px\)\);[\s\S]*min-height: max\(34px, calc\(var\(--display-control-height\) - 14px\)\);[\s\S]*padding: max\(6px, calc\(var\(--display-control-pad-y\) - 2px\)\) max\(10px, calc\(var\(--display-control-pad-x\) - 2px\)\);/);
...
assert.match(bible, /\.profile-card \.dd-user-photo \{[\s\S]*position: relative;[\s\S]*left: auto;[\s\S]*top: auto;[\s\S]*bottom: auto;[\s\S]*width: auto;[\s\S]*height: auto;[\s\S]*min-height: var\(--profile-photo-height\);[\s\S]*max-width: 45%;[\s\S]*flex: 0 1 auto;[\s\S]*align-self: stretch;[\s\S]*object-fit: cover;[\s\S]*object-position: center;[\s\S]*border-radius: 0;/);
...
assert.match(bible, /setMarqueeText\(status, navigator\.onLine \? 'Synced \\u2713' : 'Offline \\u2014 will sync later'\);/);
