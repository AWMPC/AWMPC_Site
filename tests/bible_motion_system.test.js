...
test('one quintic easing token owns application transitions', () => {
  assert.match(css, /:root \{[\s\S]*--display-nav-padding: 6px;\s*--motion-ease-fallback: cubic-bezier\(\.64, 0, \.36, 1\);\s*--motion-ease: linear\(0, 0\.00856 10%, 0\.05792 20%, 0\.16308 30%, 0\.31744 40%, 0\.5 50%, 0\.68256 60%, 0\.83692 70%, 0\.94208 80%, 0\.99144 90%, 1\);/);
  ...
  for (const [, , selector, declarations] of transitionBlocks) {
    ...
    assert.match(
      declarations,
      /transition-timing-function:\s*var\(--motion-ease-fallback\);\s*transition-timing-function:\s*var\(--motion-ease\);/,
      `${selector.trim()} declares the fallback immediately before the sampled easing`
    );
  }
