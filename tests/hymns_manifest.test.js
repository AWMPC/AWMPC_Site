...
assert.match(html, /var manifestLocation = new URL\(manifestUrl, window\.location\.href\)/);
assert.match(html, /new URL\(hymn\.file, window\.location\.href\)/);
assert.match(html, /var dataFile = hymn\.file\.replace/);
assert.match(html, /manifestLocation\.origin \+ '\/'/);
...
assert.match(html, /var fitScale = Math\.min\(maxCssWidth \/ baseViewport\.width, maxCssHeight \/ baseViewport\.height\)/);
assert.doesNotMatch(html, /function isMobilePresentationViewport\(\)/);
assert.doesNotMatch(html, /var mobilePresentation = isPresenting\(\) && isMobilePresentationViewport\(\)/);
assert.doesNotMatch(html, /stage\.classList\.toggle\('is-mobile-presenting'/);
assert.doesNotMatch(html, /\.hymns-stage\.is-mobile-presenting/);
...
assert.match(html, /var MIN_PRESENTATION_ZOOM = 1/);
assert.match(html, /var MAX_PRESENTATION_ZOOM = 3/);
assert.match(html, /var presentationZoom = 1/);
assert.match(html, /var activePointers = {}/);
assert.match(html, /\.hymns-stage\.is-presentation-zoomed:fullscreen \.hymns-canvas,\s*\.hymns-stage\.is-presentation-zoomed:-webkit-full-screen \.hymns-canvas\s*{[\s\S]*?transform: scale\(var\(--hymns-presentation-zoom, 1\)\)/);
assert.match(html, /function clampPresentationZoom\(zoom\)/);
assert.match(html, /function updatePresentationZoom\(zoom\)/);
assert.match(html, /function beginSwipe\(event\)/);
assert.match(html, /function finishSwipe\(event\)/);
assert.match(html, /function updatePinchZoom\(event\)/);
assert.match(html, /function handleGestureChange\(event\)/);
assert.match(html, /updatePresentationZoom\(pinchState\.startZoom \* zoomRatio\)/);
assert.match(html, /updatePresentationZoom\(startZoom \* event\.scale\)/);
...
assert.doesNotMatch(html, /PINCH_EXIT_RATIO/);
assert.doesNotMatch(html, /function updatePinchExit\(event\)/);
...
assert.match(html, /stage\.addEventListener\('gesturestart', beginGestureZoom\)/);
assert.match(html, /stage\.addEventListener\('gesturechange', handleGestureChange\)/);
assert.match(html, /stage\.addEventListener\('gestureend', endGestureZoom\)/);
...
assert.doesNotMatch(html, /function registerHymnsServiceWorker\(\)/);
assert.doesNotMatch(html, /navigator\.serviceWorker\.register\('hymns-sw\.js'/);
assert.doesNotMatch(html, /function ensureHymnsManifestLink\(\)/);
assert.doesNotMatch(html, /id="hymnInstall"/);
